...
public class StatBuilder implements KafkaStreamsInfrastructureCustomizer {
/*
    ...
    private Duration windowSizeInMs;
    //@Value("#{T(java.time.Duration).ofMillis(${stat.punctuation.step})}")
    private Duration punctuationStepInMs;
*/
    ...
    private String INPUT_TOPIC_NAME;
    private String SINK_TOPIC_NAME;
    private final boolean uniqueIp;
    private final Duration windowSizeInMs;
    private final Duration punctuationStepInMs;

    public StatBuilder(String INPUT_TOPIC_NAME, String SINK_TOPIC_NAME,
                       boolean uniqueIp, Duration windowSizeInMs, Duration punctuationStepInMs) {
        this.INPUT_TOPIC_NAME = INPUT_TOPIC_NAME;
        this.SINK_TOPIC_NAME = SINK_TOPIC_NAME;
        this.uniqueIp = uniqueIp;
        this.windowSizeInMs = windowSizeInMs;
        this.punctuationStepInMs = punctuationStepInMs;
    }
    ...
    public void configureBuilder(@NonNull StreamsBuilder streamsBuilder) {
        //TODO ВЕРНУТЬ!!!!!!!!!!!!!!
        //testMissingAggregatedBuilder(streamsBuilder);
        //testMissingWindowedBuilder(streamsBuilder, windowSizeInMs, punctuationStepInMs);
    ...
    private void testMissingWindowedBuilder(StreamsBuilder streamsBuilder,
        ...
        KStream<String, StatOutDto> stream = streamsBuilder.stream(INPUT_TOPIC_NAME,
                ...
                /*.process(() -> new DistinctFilterTestProcessor(INPUT_TOPIC_NAME, punctuationDuration),
                        STORE_NAME_DISTINCT)*/
    ...
    //TODO Тест, УБРАТЬ !!!!!!!!!!!!
    private void testMissingAggregatedBuilder(StreamsBuilder streamsBuilder) {
        KeyValueBytesStoreSupplier distinctStoreSupplier = Stores
                .inMemoryKeyValueStore(STORE_NAME_DISTINCT); //для отсечения дублей
        //создаем строителей хранилищ
        StoreBuilder<KeyValueStore<String, HashSet<String>>> distinctStoreBuilder = Stores
        ...
        KStream<String, StatOutDto> stream = streamsBuilder.stream(INPUT_TOPIC_NAME,
                        Consumed.with(Serdes.String(), new JsonSerde<>(StatPartDto.class).typeMapper(getTypeMapper()))
                                .withOffsetResetPolicy(Topology.AutoOffsetReset.LATEST))
                //.withOffsetResetPolicy(Topology.AutoOffsetReset.EARLIEST))
                .process(() -> new DistinctFilterTestProcessor(INPUT_TOPIC_NAME, punctuationStepInMs),
                        STORE_NAME_DISTINCT)
                ...
                .groupByKey(Grouped.with(Serdes.String(),
                        new JsonSerde<>(StatOutDto.class)))
                .aggregate(() -> new StatOutDto(INPUT_TOPIC_NAME, "", 0),
                        (k, v, a) -> {a.setUri(k); a.inc(); return a;},
                        Materialized
                ...
                .toStream()
                .process(() -> new MissingProcessor(punctuationStepInMs, "Второй процессор"))
                .process(() -> new MissingProcessor(punctuationStepInMs, "Третий процессор"))
                //.process(() -> new MissingProcessor(punctuationStepInMs, "Четвертый процессор"))
                ;
        //выводим выходные записи
        ...
        */
        ...
        KStream<String, StatOutDto> stream = streamsBuilder.stream(INPUT_TOPIC_NAME,
                        Consumed.with(Serdes.String(), new JsonSerde<>(StatPartDto.class).typeMapper(getTypeMapper()))
                                ...
                                //.withOffsetResetPolicy(Topology.AutoOffsetReset.EARLIEST))
                //.filter((k, v) -> !k.contains("mocked"))
                //.process(() -> new DistinctFilterTestProcessor(INPUT_TOPIC_NAME, punctuationStepInMs), STORE_NAME_DISTINCT)
                .process(() -> new DistinctFilterProcessor(INPUT_TOPIC_NAME), STORE_NAME_DISTINCT)
                //.mapValues(v -> new StatOutDto(INPUT_TOPIC_NAME, v.getUri(), 1))
